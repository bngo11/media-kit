...
SRC_URI="https://api.github.com/repos/AcademySoftwareFoundation/OpenImageIO/tarball/v3.0.11.0 -> openimageio-3.0.11.0.tar.gz"
...
src_prepare() {
	...
	if has_version media-libs/openexr:3 ; then
		for file in $(grep -rl '<OpenEXR/' "${S}") ; do
			sed -i -e 's#<OpenEXR/#<OpenEXR-3/#g' "$file"
		done
	fi
	cmake_src_prepare
